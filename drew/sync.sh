...
# 10. Sync ALL remaining JSON files from Drew's machine (catch anything custom transforms miss)
rsync -az --include="*.json" --exclude="*" -e "ssh -o ConnectTimeout=10" $REMOTE:$REMOTE_CLAWD/dashboard/data/ "$DATA_DIR/" 2>/dev/null
